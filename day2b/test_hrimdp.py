                    ...
                    for human_y in range(self.cols):
                        for human_request in (0,1):
                            if not self.gridworld.is_wall(robot_x, robot_y):
                                ...
                                # Start codes here
                                # print("current stat is: ", state)
                                reward[state] = -1
                                if human_request == 1:
                                    if abs(robot_x - human_x) + abs(robot_y - human_y) <= 1:
                                        reward[state] = 10
                                else:
                                    if abs(robot_x - human_x) + abs(robot_y - human_y) <= 1:
                                        reward[state] = -10
                                    elif abs(robot_x - human_x) + abs(robot_y - human_y) == 2:
                                        reward[state] = -5
...
#########################################################################
    ...
    def calculate_T(self, state, action):
        if action:
            ...
            return [
                (0.5, self.execute_one_step(state, action, 'S')),
...
##########################################################################
    ...
    def human_execute_one_step(self, state, action):
        ...
        if self.gridworld.is_wall(next_cell[0], next_cell[1]):
            ...
            if action == 'T':
                if state[4] == 0:  # Toggle Request from Human
                    next_state = [next_cell, 1]
                else:
                    next_state = [next_cell, 0]
            else:
                next_state = [next_cell, state[4]]

            return (state[0], state[1], next_state[0][0], next_state[0][1], next_state[1])
            #return (next_state[0][0], next_state[0][1], next_cell[0], next_cell[1], next_state[1])

'''    
if self.gridworld.is_wall(next_cell[0], next_cell[1]):
    return state
else:
    if action == 'T' and state[4] == 0: # Toggle Request from Human and current status is 'False'
        next_state = [next_cell, 1]

    elif action == 'T' and state[4] == 1: # Toggle Request from Human and current status is 'True'
        next_state = [next_cell, 0]
    ...
    return (next_state[0][0], next_state[0][1], next_cell[0], next_cell[1], next_state[1])
    #return (state[2], state[3], next_cell[0], next_cell[1], state[4])
'''
'''
